...
class HomeController extends GetxController {
  ...
  Future<void> loadUsers() async {
    ...
    if (_users!.result.data.length > 0) {
      users.value = _users;
      users.refresh();
  ...
  void _saveUserInfo(UsersResponse users) {
    ...
    var index = random.nextInt(users.result.data.length);
    user.value = users.result.data[index];
    var prefs = Get.find<SharedPreferences>();
    prefs.setString(StorageConstants.userInfo, users.result.data.isNotEmpty.toString());
